...
from .state import CulpritState, GameState, JunctionCheck, PoliceBlock, WitnessBatch
...
def question_witness(state: GameState, witness_id: str, question: str) -> tuple[GameState, str]:
    ...
    batch = next((batch for batch in state.witness_batches if any(w.witness_id == witness_id for w in batch.witnesses)), None)
    if batch and not batch.individual_review_allowed:
        return state, "Too many witnesses in this batch. Narrow the notice before individual questioning."
